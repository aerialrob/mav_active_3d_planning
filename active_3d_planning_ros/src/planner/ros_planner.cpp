...
namespace active_3d_planning
{
    namespace ros
    {
            ...
            : OnlinePlanner(factory, param_map), nh_(nh), nh_private_(nh_private)
        {
            ...
            planner_status_pub_ = nh_.advertise<std_msgs::Int16>("status", 100);
            odom_sub_ = nh_.subscribe("odometry", 1, &RosPlanner::odomCallback, this);
            global_goal_sub_ = nh_.subscribe("global_goal", 1, &RosPlanner::globalGoalCallback, this);
            ...
            ROS_INFO_STREAM("\n******************** Initialized Planner ********************\n"
                            ...
                            << param_map->at("param_namespace") << "' with parameters:" << param_map->at("verbose_text"));
        }
        ...
        void RosPlanner::initializePlanning()
        {
            ...
            global_goal_position_ = Eigen::Vector3d{0.0, 0.0, 0.0};
            trajectory_generator_->setGlobalGoal(&global_goal_position_);
            trajectory_evaluator_->setGlobalGoal(&global_goal_position_);
            //trajectory_evaluator_->initializeObservedVolume();
        }
        ...
        void RosPlanner::planningLoop()
        {
            ...
            while (::ros::ok())
            {
                if (planning_ && received_first_goal_)
                {
                    loopIteration();
                }
                if (p_log_performance_)
        ...
        void RosPlanner::globalGoalCallback(const geometry_msgs::PoseStamped &msg)
        {
            global_goal_position_ = Eigen::Vector3d(msg.pose.position.x, msg.pose.position.y,
                                                    msg.pose.position.z);
            std::cout << "Got new global goal " << global_goal_position_ << "\n";
        ...
        void RosPlanner::publishVisualization(const VisualizationMarkers &markers)
        {
            ...
            if (markers.getMarkers()[0].action == VisualizationMarker::OVERWRITE)
            {
                ...
                std::map<std::string, int>::iterator it = visualization_overwrite_counter_.find(
                    markers.getMarkers()[0].ns);
                int count = 0;
        ...
        void RosPlanner::publishStatus(std::int16_t &status)
        {
            //std_msgs::Int16 status_msg = status;
        ...
        void RosPlanner::printInfo(const std::string &text)
        {
        ...
        void RosPlanner::printError(const std::string &text)
        {
